...
import { PARAM_FIELDS } from './parameter-fields';
  ...
  <div class="tray-scroll evidence-list">
    @if (store.active().extraction; as result) {
      ...
      @for (field of result.campos; track $index) {
        <article class="evidence-row"><div><strong>{{ label(field.campo) }}</strong><span>{{ field.valor }}</span></div><blockquote>{{ field.trecho }}</blockquote>
          <button class="text-button" type="button" (click)="showSource(field)">{{ field.documento }} · página {{ field.pagina }}</button>
        </article>
...
export class EvidencePanelComponent {
  ...
  showSource(field: FieldEvidence): void {const document = this.store.documents().find(item => item.nome === field.documento); if (document) {this.store.selectedDocument.set(document.identificador);this.store.pdfPage.set(field.pagina);this.sourceSelected.emit();}}
}
